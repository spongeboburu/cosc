...
static struct cosc_serial reader;
...
#ifndef COSC_NOARRAY
// FIXME: what is the garbage at the end??
...
static void test_empty_bundle_noprefix(void **state)
{
    ...
    assert_int_equal(cosc_reader_start_bundle(&reader, &timetag), 16);
#ifndef COSC_NOINT64
    assert_int_equal(timetag, 0x12345678);
#else
    assert_int_equal(COSC_64BITS_LO(&timetag), 0x12345678);
#endif
    assert_int_equal(cosc_reader_end_bundle(&reader), 0);
    assert_int_equal(cosc_serial_get_size(&reader), 16);
    assert_int_equal(cosc_reader_start_bundle(&reader, NULL), COSC_EPSIZEFLAG);
}
...
static void test_empty_bundle_prefix(void **state)
{
    ...
    assert_int_equal(cosc_reader_start_bundle(&reader, &timetag), 20);
    assert_int_equal(cosc_reader_end_bundle(&reader), 0);
    assert_int_equal(cosc_serial_get_size(&reader), 20);
    assert_int_equal(cosc_reader_start_bundle(&reader, &timetag), 20);
    assert_int_equal(cosc_reader_end_bundle(&reader), 0);
    assert_int_equal(cosc_serial_get_size(&reader), 40);
}
...
static void test_empty_message_noprefix(void **state)
{
    ...
    assert_int_equal(cosc_reader_start_message(&reader, &address, &address_n, &typetag, &typetag_n), 8);
    ...
    assert_int_equal(cosc_serial_get_size(&reader), 8);
    assert_int_equal(cosc_reader_start_message(&reader, &address, &address_n, &typetag, &typetag_n), COSC_EPSIZEFLAG);
}
...
static void test_empty_message_prefix(void **state)
{
    ...
    assert_int_equal(cosc_reader_start_message(&reader, &address, &address_n, &typetag, &typetag_n), 12);
    assert_string_equal(address, "abc");
    assert_int_equal(address_n, 3);
    assert_string_equal(typetag, ",");
    assert_int_equal(typetag_n, 1);
...
static void test_bundle_empty_messages_noprefix(void **state)
{
    ...
    assert_int_equal(typetag_n, 1);
    assert_int_equal(cosc_reader_end_message(&reader, 1), 0);
    assert_int_equal(cosc_serial_get_size(&reader), 16 + 12);
    assert_int_equal(cosc_reader_start_message(&reader, 0, 0, 0, 0), 12);
    assert_int_equal(cosc_reader_end_message(&reader, 1), 0);
    assert_int_equal(cosc_serial_get_size(&reader), 16 + 24);
    assert_int_equal(cosc_reader_start_message(&reader, 0, 0, 0, 0), 12);
    assert_int_equal(cosc_reader_end_message(&reader, 1), 0);
    assert_int_equal(cosc_serial_get_size(&reader), 16 + 36);
    assert_int_equal(cosc_reader_start_message(&reader, 0, 0, 0, 0), 12);
    assert_int_equal(cosc_reader_end_message(&reader, 1), 0);
    assert_int_equal(cosc_reader_end_bundle(&reader), 0);
    assert_int_equal(cosc_serial_get_size(&reader), 16 + 48);
    assert_int_equal(cosc_reader_start_bundle(&reader, 0), COSC_EPSIZEFLAG);
}
...
static void test_message_noarray(void **state)
{
    ...
    assert_int_equal(cosc_reader_start_message(&reader, &address, &address_n, &typetag, &typetag_n), 28);
    assert_int_equal(cosc_serial_get_size(&reader), 28);
    assert_int_equal(cosc_serial_get_msgtype(&reader), 'i');
    assert_int_equal(cosc_reader_int32(&reader, &value.i), 4);
    assert_int_equal(cosc_serial_get_size(&reader), 32);
    assert_int_equal(cosc_serial_get_msgtype(&reader), 'f');
    assert_int_equal(cosc_reader_float32(&reader, &value.f), 4);
    assert_int_equal(cosc_serial_get_size(&reader), 36);
    assert_int_equal(cosc_serial_get_msgtype(&reader), 's');
    assert_int_equal(cosc_reader_string(&reader, 0, 0, &value.s.length), 4);
    assert_int_equal(cosc_serial_get_size(&reader), 40);
    assert_int_equal(cosc_serial_get_msgtype(&reader), 'b');
    assert_int_equal(cosc_reader_blob(&reader, 0, 0, &value.b.b, &value.b.size), 8);
    assert_int_equal(cosc_serial_get_size(&reader), 48);
    assert_int_equal(cosc_serial_get_msgtype(&reader), 'h');
    assert_int_equal(cosc_reader_int64(&reader, &value.h), 8);
    assert_int_equal(cosc_serial_get_size(&reader), 56);
    assert_int_equal(cosc_serial_get_msgtype(&reader), 't');
    assert_int_equal(cosc_reader_uint64(&reader, &value.t), 8);
    assert_int_equal(cosc_serial_get_size(&reader), 64);
    assert_int_equal(cosc_serial_get_msgtype(&reader), 'd');
    assert_int_equal(cosc_reader_float64(&reader, &value.d), 8);
    assert_int_equal(cosc_serial_get_size(&reader), 72);
    assert_int_equal(cosc_serial_get_msgtype(&reader), 'S');
    assert_int_equal(cosc_reader_string(&reader, 0, 0, &value.s.length), 4);
    assert_int_equal(cosc_serial_get_size(&reader), 76);
    assert_int_equal(cosc_serial_get_msgtype(&reader), 'c');
    assert_int_equal(cosc_reader_char(&reader, &value.c), 4);
    assert_int_equal(cosc_serial_get_size(&reader), 80);
    assert_int_equal(cosc_serial_get_msgtype(&reader), 'r');
    assert_int_equal(cosc_reader_uint32(&reader, &value.r), 4);
    assert_int_equal(cosc_serial_get_size(&reader), 84);
    assert_int_equal(cosc_serial_get_msgtype(&reader), 'm');
    assert_int_equal(cosc_reader_midi(&reader, value.m), 4);
    assert_int_equal(cosc_serial_get_size(&reader), 88);
    assert_int_equal(cosc_serial_get_msgtype(&reader), 'T');
    ...
    assert_int_equal(cosc_reader_skip(&reader), 0);
    assert_int_equal(cosc_serial_get_size(&reader), 88);
    assert_int_equal(cosc_serial_get_msgtype(&reader), 'N');
    assert_int_equal(cosc_reader_skip(&reader), 0);
    assert_int_equal(cosc_serial_get_size(&reader), 88);
    assert_int_equal(cosc_serial_get_msgtype(&reader), 'I');
    assert_int_equal(cosc_reader_skip(&reader), 0);
    assert_int_equal(cosc_serial_get_size(&reader), 88);
    assert_int_equal(cosc_serial_get_msgtype(&reader), 0);
    assert_int_equal(cosc_reader_end_message(&reader, 1), 0);
    assert_int_equal(cosc_serial_get_size(&reader), 88);
}
...
static void test_message_array(void **state)
{
    ...
    assert_int_equal(cosc_reader_start_message(&reader, &address, &address_n, &typetag, &typetag_n), 16);
    assert_int_equal(cosc_serial_get_size(&reader), 16);
    assert_int_equal(cosc_serial_get_msgtype(&reader), 'i');
    assert_int_equal(cosc_reader_int32(&reader, &value.i), 4);
    assert_int_equal(cosc_serial_get_size(&reader), 20);
    cosc_int32 sz = cosc_serial_get_size(&reader);
    for (int i = 0; i < 10; i++)
    {
        assert_int_equal(cosc_serial_get_msgtype(&reader), '[');
        assert_int_equal(cosc_reader_skip(&reader), 0);
        for (int j = 0; j < 3; j++)
        {
            assert_int_equal(cosc_serial_get_msgtype(&reader), 'f');
            assert_int_equal(cosc_reader_float32(&reader, &value.f), 4);
            sz += 4;
            assert_int_equal(cosc_serial_get_size(&reader), sz);
        }
        assert_int_equal(cosc_serial_get_msgtype(&reader), ']');
        assert_int_equal(cosc_reader_repeat(&reader), 0);
    }
    assert_int_equal(cosc_reader_end_message(&reader, 1), 0);
}
#endif
...
{
    cosc_reader_setup(&reader, message_noarray, sizeof(message_noarray), levels, level_max, COSC_SERIAL_PSIZE);
    assert_int_equal(cosc_reader_start_message(&reader, 0, 0, 0, 0), 28);
    assert_int_equal(cosc_reader_end_message(&reader, 1), 60);
    assert_int_equal(cosc_serial_get_size(&reader), 88);
}

// FIXME: perhaps the message is broken?
// #ifndef COSC_NOARRAY
// static void test_message_unfinished_array(void **state)
// {
//     cosc_reader_setup(&reader, message_array, sizeof(message_array), levels, level_max, COSC_SERIAL_PSIZE);
//     assert_int_equal(cosc_reader_start_message(&reader, 0, 0, 0, 0), 16);
//     assert_int_equal(cosc_reader_end_message(&reader, 0), 120);
//     assert_int_equal(cosc_serial_get_size(&reader), 32);
// }
// #endif
